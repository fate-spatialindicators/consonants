# this is all derived from code/run_wc_models.r
...
for(i in 1:nrow(df)) {
  ...
  if(class(spde) != "try-error") {
    ...
    if(class(m)!="try-error") {
      ...
      for(j in 2:nrow(var)) {
        fit <- nlme::gls(Estimate ~ 1, 
          data = dplyr::filter(diff_est,year==unique(epsilon_st$year)[j]),
          correlation = corGaus(form = ~lat + lon, nugget = TRUE),
          method = "REML")
        var$diff_range[j] = attr(fit$apVar,"Pars")[1]
        var$diff_nugget[j] = attr(fit$apVar,"Pars")[2]
      }
      saveRDS(var,file=paste0("output/wc/summary_extremes_",i,".rds"))
      saveRDS(m, file=paste0("output/wc/model_extremes_",i,".rds"))
      #sd_report <- summary(m$sd_report)
      #params <- as.data.frame(sd_report[grep("quadratic", row.names(sd_report)), ])
    }
  } # end try on spde
